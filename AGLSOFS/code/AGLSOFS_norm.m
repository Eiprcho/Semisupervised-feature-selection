function [W,objs] = AGLSOFS_norm(X,YL,param)
...
for it=1:max_ite
    ...
    for ji = 1:n
        S(ji,:) = EProjSimplex_new((-1/(2*la2))*dist(ji,:));
    end
    S = (S+S')/2;
    L = diag(sum(S,2)) - S; 
    ...
    o2 = 2*beta*gama*trace(W'*X'*L*X*W)+2*beta*(1-gama)*trace(X'*L*X)+beta*la2*trace(S'*S);
    objs(it) = o1 + o2;
